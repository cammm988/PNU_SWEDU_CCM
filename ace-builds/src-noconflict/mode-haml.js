...
var constantOtherSymbol = exports.constantOtherSymbol = {
    token : "constant.test.symbol.ruby", // symbol
...
var RubyHighlightRules = function() {
    ...
    this.$rules = {
        "start": [
            ...
            [{
                token: ["constant.test.symbol.ruby", "string.start"],
                regex: /(:)?(")/,
                push: [{
                    token: "constant.language.escape",
                    regex: escapedChars
                ...
                token: "constant.test.symbol.ruby", //doesn't see any differences between symbols and array of symbols in highlighting
        ...
        "sStateWithoutInterpolation": [{
            token: "constant.test.symbol.ruby",// excluded nested |^% due to difficulty in realization
            regex: /[(\[<{]/, onMatch: function (val, state, stack) {
                if (stack.length && val === stack[0]) {
                    stack.unshift(val, state);
                    return this.token;
                }
                return "constant.test.symbol.ruby";
            }
        }, {
            token: "constant.test.symbol.ruby",
            regex: /[)\]>}^|%]/, onMatch: function (val, state, stack) {
                if (stack.length && val === closeParen[stack[0]]) {
                    stack.shift();
                    this.next = stack.shift();
            ...
            defaultToken: "constant.test.symbol.ruby"
        }],
