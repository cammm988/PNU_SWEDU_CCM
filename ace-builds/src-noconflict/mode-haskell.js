    ...
    this.$rules = { start:
       [ { token:
         ...
         { token: 'constant.language.empty-list.haskell',
           regex: '\\[\\]' },
         { token: 'keyword.test.haskell',
           regex: '\\b(module|signature)\\b',
           push:
            [ { token: 'keyword.test.haskell', regex: '\\bwhere\\b', next: 'pop' },
              { include: '#module_name' },
         ...
         { token: 'keyword.test.haskell',
           ...
           push:
            [ { token: 'keyword.test.haskell',
              ...
              { token: 'entity.test.inherited-class.haskell',
                regex: '[A-Z][A-Za-z_\']*' },
              { token: 'variable.test.generic-type.haskell',
         ...
         { token: 'keyword.test.haskell',
           ...
           push:
            [ { token: 'meta.import.haskell', regex: '$|;|^', next: 'pop' },
              { token: 'keyword.test.haskell', regex: 'qualified|as|hiding' },
              { include: '#module_name' },
              { include: '#module_exports' },
              { defaultToken: 'meta.import.haskell' } ] },
         { token: [ 'keyword.test.haskell', 'meta.deriving.haskell' ],
           regex: '(deriving)(\\s*\\()',
           push:
         ...
         { token:
            [ 'meta.function.type-declaration.haskell',
              ...
              'meta.function.type-declaration.haskell',
              'keyword.test.double-colon.haskell' ],
         ...
         { token: 'constant.test.haskell', regex: '\\b[A-Z]\\w*\\b' },
         { include: '#comments' },
       ...
       [ { token: 'meta.declaration.exports.haskell',
           ...
           push:
            [ { token: 'meta.declaration.exports.haskell.end',
              ...
              { token: 'punctuation.separator.comma.haskell', regex: ',' },
              { include: '#infix_op' },
              { token: 'meta.test.unknown.haskell',
                regex: '\\(.*?\\)',
                comment: 'So named because I don\'t know what to call this.' },
              { defaultToken: 'meta.declaration.exports.haskell.end' } ] } ],
      '#module_name':
       [ { token: 'support.test.module.haskell',
           regex: '[A-Z][A-Za-z._\']*' } ],
      '#pragma':
       [ { token: 'meta.preprocessor.haskell',
           ...
           push:
            [ { token: 'meta.preprocessor.haskell',
                regex: '#-\\}',
                next: 'pop' },
              { token: 'keyword.test.preprocessor.haskell',
                regex: '\\b(?:LANGUAGE|UNPACK|INLINE)\\b' },
              { defaultToken: 'meta.preprocessor.haskell' } ] } ],
      '#type_signature':
       [ { token:
            [ 'meta.class-constraint.haskell',
              'entity.test.inherited-class.haskell',
              'meta.class-constraint.haskell',
              'variable.test.generic-type.haskell',
              'meta.class-constraint.haskell',
              'keyword.test.big-arrow.haskell' ],
           regex: '(\\(\\s*)([A-Z][A-Za-z]*)(\\s+)([a-z][A-Za-z_\']*)(\\)\\s*)(=>)' },
         { include: '#pragma' },
         { token: 'keyword.test.arrow.haskell', regex: '->' },
         { token: 'keyword.test.big-arrow.haskell', regex: '=>' },
